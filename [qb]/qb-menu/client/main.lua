...
-- Events
RegisterNetEvent('pilot:rentPlane', function(model)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)

    if Player.Functions.RemoveMoney('bank', Config.Rental.Deposit) then
        TriggerClientEvent('QBCore:Notify', src, "Deposit paid", "success")
        TriggerClientEvent('pilot:rentSuccess', src)
    else
        TriggerClientEvent('QBCore:Notify', src, "Not enough money", "error")
    end
end)

QBCore.Functions.CreateCallback('pilot:rentPlane', function(source, cb)
...
RegisterNetEvent('pilot:client:spawnPlane', function(data)
    if CurrentPlane then
        QBCore.Functions.Notify("Return current plane first", "error")
        return
    end

    QBCore.Functions.TriggerCallback('pilot:rentPlane', function(success)
        if not success then return end

        local model = data.model
        local coords = Config.Rental.Spawn

        RequestModel(model)
        while not HasModelLoaded(model) do Wait(0) end

        CurrentPlane = CreateVehicle(model, coords.x, coords.y, coords.z, coords.w, true, false)
        TaskWarpPedIntoVehicle(PlayerPedId(), CurrentPlane, -1)
    end, model)
end)
RegisterNetEvent('pilot:client:returnPlane', function()
    if not CurrentPlane then return end

    DeleteVehicle(CurrentPlane)
    CurrentPlane = nil

    TriggerServerEvent('pilot:returnDeposit')
end)

RegisterNetEvent('pilot:returnDeposit', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)

    Player.Functions.AddMoney('bank', Config.Rental.Deposit)
    TriggerClientEvent('QBCore:Notify', src, "Deposit returned", "success")
end)
